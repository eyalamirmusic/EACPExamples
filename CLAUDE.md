...
- **No `MIRO_EXPORT_EVENT` macro.** The push side of the bridge (`bridge.send<T>(event, payload)` on C++, `window.eacp.on<T>(event, handler)` on JS) works, but events aren't part of the schema codegen — JS receives `unknown` payloads unless you hand-write the TS interfaces. Mirrors the `MIRO_EXPORT_COMMAND` shape; should be straightforward to add.
- **Schema-export tool requires linking the body of every command, including transitive deps.** `MIRO_EXPORT_COMMAND(fn)` expands to `registerCommand<&fn>(...)` — the address of `fn` is taken as a non-type template parameter, so the linker requires `fn`'s body. Anything `fn` calls must also link into the schema-export tool. For PhysicsDemo this forces `World.cpp` and `reactphysics3d` into a tool that only emits TS interfaces — wasteful build and surprising. Possible fixes: (a) Miro could provide a `MIRO_TYPE_EXPORT_ONLY` mode where the macro emits a type-only registration without taking `&fn`, or (b) `eacp_add_webview_app` could expose `SCHEMA_EXTRA_LIBRARIES` / `SCHEMA_EXTRA_SOURCES` arguments instead of forcing callers to mutate `${TARGET}Schema` after the fact.
- **`Threads::Timer(cb, intervalHz)` takes Hz, not period.** `Timer{cb, 1}` reads naturally as "fire once per second" but actually means "1 Hz", which only happens to coincide. A `Timer{cb, 60}` for 60 fps and `Timer{cb, 0.5}` for once-every-two-seconds aren't expressible. Either rename to `frequencyHz`, or accept `std::chrono::duration` for an unambiguous spelling.
